...
from llava.dataset.utils import make_web_data_module
...
class DataArguments:
    ...
    shard_pattern: str = field(default=None,
                               metadata={"help": "Name pattern for shard files"})
    shard_size: int = field(default=1000,
                            metadata={"help": "Number of samples in each shard"})
...
def train(attn_implementation=None):
    ...
    #                                           data_args=data_args)
    data_module = make_web_data_module(tokenizer=tokenizer,
                                       data_args=data_args)
    world_size = torch.cuda.device_count()
    max_steps = int( data_module['train_dataset'].est_len // (training_args.per_device_train_batch_size * world_size) * training_args.num_train_epochs )
